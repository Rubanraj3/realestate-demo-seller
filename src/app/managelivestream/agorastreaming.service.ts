...
export class AgorastreamingService {
  ...
  rtc: any = {
    client: AgoraRTC.createClient({ mode: 'rtc', codec: 'vp8', role: "host" }),
  ...
  async localUser(token: any, uuid: any, type: any, channel: any, req: any) {
    ...
    // this.users = await this.rtc.client.getRemoteUsers();
    if (type != 'live') {
  ...
  async switch_cam(deviceId: any) {
    console.log(this.users, 76543211, await this.rtc.client)
