...
export class SocketioService {
  ...
  send_message(body: any) {
    return this.socket.emit('groupchathost_demo', body);
  }
  ...
  _stream_joins(channel: any): Observable<any> {
    return new Observable<any>(observer => {
      this.socket.on(channel + '_stream_joins', (data: any) => {
        observer.next(data);
      });
      return () => {
        this.socket.disconnect();
  ...
  _buyer_registor(channel: any): Observable<any> {
    return new Observable<any>(observer => {
