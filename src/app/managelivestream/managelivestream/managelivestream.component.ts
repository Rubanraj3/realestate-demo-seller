...
export class ManagelivestreamComponent {
  ...
  copy_now() {
    ...
    if (this.my_stream.type == 'demo') {
      navigator.clipboard.writeText('https://ag23.site/b/' + this.share_icon);
    }
    else {
      navigator.clipboard.writeText('https://meet.warmy.co.in/demo/' + this.share_icon);
    }
    setTimeout(() => {
